...
export default function LoginPage() {
  ...
  const handleGoogleLogin = () => {
    ...
    if (!clientId) {
      // Mock code triggers developer bypass flow
      console.warn('[OAuth] Google Client ID not configured. Using mock flow for development.');
      const mockCode = `mock-google-${Math.random().toString(36).substring(2, 10)}`;
      window.location.href = `/auth/callback/google?code=${mockCode}`;
      return;
    }

    // This redirect_uri MUST EXACTLY match what is registered in Google Cloud Console
    // and what the backend sends when exchanging the code for tokens
    const redirectUri = 'http://localhost:3000/auth/callback/google';
    const googleAuthUrl = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=email%20profile&prompt=consent`;
  ...
  const handleGitHubLogin = () => {
    ...
    if (!clientId) {
      // Mock code triggers developer bypass flow
      console.warn('[OAuth] GitHub Client ID not configured. Using mock flow for development.');
      const mockCode = `mock-github-${Math.random().toString(36).substring(2, 10)}`;
      window.location.href = `/auth/callback/github?code=${mockCode}`;
      return;
    }
    ...
    <div className="flex items-center justify-center min-h-[calc(100vh-4rem)]">
      <div className="w-full max-w-md p-8 space-y-6 border rounded-lg bg-card text-card-foreground shadow-lg backdrop-blur-md">
        <div className="text-center">
          ...
          <p className="text-muted-foreground mt-2">Log in to continue battling</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <button
            ...
            className="flex items-center justify-center py-2 px-4 border rounded-md hover:bg-accent hover:text-accent-foreground transition-all font-medium text-sm"
          >
          ...
          <button
            ...
            className="flex items-center justify-center py-2 px-4 border rounded-md hover:bg-accent hover:text-accent-foreground transition-all font-medium text-sm"
          >
            <svg className="mr-2 h-4 w-4" aria-hidden="true" focusable="false" viewBox="0 0 496 512">
        ...
        {(!process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || !process.env.NEXT_PUBLIC_GITHUB_CLIENT_ID) && (
          <div className="text-center text-xs text-muted-foreground border border-dashed border-muted p-2 rounded">
            Local Developer Bypass enabled.<br />OAuth buttons will run mock flow.
          </div>
        )}
